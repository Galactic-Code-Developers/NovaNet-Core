...
contract NovaNetValidator is Ownable, ReentrancyGuard {
    ...
    /// @notice Registers a new validator by staking a minimum amount.
    ...
    /// @notice Records block production by an active validator.
    ...
    function getValidatorInfo(address _validator) external view returns (
        uint256 stakedAmount, uint256 reputationScore, uint256 lastBlockProduced, bool isActive
    ) {
        Validator storage validator = validators[_validator];
        return (validator.stakedAmount, validator.reputationScore, validator.lastBlockProduced, validator.isActive);
    }
}
